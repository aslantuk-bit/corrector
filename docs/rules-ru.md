...
Шум по корпусу: около 11 срабатываний на акт — в актах инициалы почти всегда без пробела; правило верное, категорию «Типографика» можно выключить галочкой.
...
## rep.near — Повтор слова в соседних предложениях (отключено по умолчанию)
Категория: style · Уровень: hint · Язык: ru

Шум по корпусу: срабатывает в 90 % актов (около 20 раз на акт), потому что юридический текст повторяет термины намеренно.
Правило есть в коде (`corrector/rules/repeats.py`), но в набор по умолчанию не входит; таблица не проверяется тестами.
...
| Отчёт содержит сведения об оценщике Саржановой. Саржанов подтвердил. Саржанова явилась. | ✓ |
| Присутствовали Утениязов Орынбай Утениязович и Болат Утениязов. | ✓ |
...
| Что суд решил. | ✓ |
| Стороне, в пользу которой состоялось решение, суд присуждает расходы. | ✓ |
| Закон, согласно которому основанием служит заявление. | ✓ |
| Требования, а также если они не относятся к делу. | ✓ |

Шум по корпусу: после правки исключений (предлоги, «также», «согласно», запятая в трёх предыдущих токенах) — см. docs/corpus-report.md.
