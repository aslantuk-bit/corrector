...
## Что показал первый прогон на сборщике (15.09.2026)

Всё до запуска собранной программы прошло с первого раза, включая кириллическое имя exe. Приложенная Java
ответила «Error: could not find java.dll»: запускатель Java на Windows не читает путь с символами вне
системной кодовой страницы (на сборщике английская Windows, cp1252). В `probes.py` добавлены
`active_code_page`, `path_fits_code_page`, `default_fallback_roots`, `copy_java_to_safe_dir`, в `report.py` —
`check_java` (прямой запуск, при отказе копия в `%PUBLIC%`/`%ProgramData%`/`%SystemDrive%\Temp`/`%TEMP%`),
отчёт получил строки «Кодовая страница», «Java напрямую», «Копия Java». Спецификация дополнена (раздел 5.1).

Второе открытие: у токена workflow в новом репозитории права только на чтение, шаг релиза получал 403 «Resource not accessible by integration» — в workflow добавлено `permissions: contents: write`, тэг v0.1.0 перенесён на исправленный коммит.
